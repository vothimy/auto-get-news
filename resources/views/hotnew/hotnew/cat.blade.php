    ...
    <div class="main-content">
      <div class="column-two-third">
       ...
       <div class="outertight m-t-no">
        <div class="badg">
          <p><a href="">Nổi bật.</a></p>
        </div>
        <div class="flexslider">
          <ul class="slides">
            <li>
              <img src="{{ $adminfiles }}/{{ $picture }}" alt="MyPassion" style="width:300px;height:200px;" />
            </li>
          </ul>
        </div>
        <h6 class="regular"><a href="{{ route('public.hotnew.detail',['slug'=>$slug,'id'=>$id]) }}">{{ $name}}.</a></h6>
      ...
      <div class="outertight m-r-no m-t-no">
        ...
        <h6 class="regular"><a href="{{ route('public.hotnew.detail',['slug'=>$slug,'id'=>$id]) }}">{{ $name }}.</a></h6>
        <span class="meta">{{ $create }}.   |   <a href="">Lượt đọc: {{ $read }}.</a>   |   <a href="">Bình luận : {{ $i }}.</a></span>
        <p>{!! $preview !!}...</p>
      </div>
      @endforeach
      <div class="outerwide">
        <ul class="block2">
          @foreach($arItems3 as $item)
          <?php 
            $id = $item->id_news;
            $name1 = $item->name;
            $preview = $item->preview;
